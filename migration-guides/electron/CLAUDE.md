...
```json
{
  ...
  "notes": "BrightSign loads HTML via autorun.brs configuration, not programmatically. Chromium media player is supported on Series 5 onwards only. See ../media-player-selection.md for cases where the BrightSign media player should be used instead (Series 4, HDMI input, sync, UDP/RTP, chroma key)."
}
```
