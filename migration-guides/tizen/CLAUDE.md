...
4. Separately, always ask whether this BrightSign deployment has interactive hardware (touchscreen, GPIO buttons/sensors, USB HID remote receiver, etc.) before deciding whether media should loop at the end. A Tizen app being remote-interactive is NOT evidence the BrightSign deployment is interactive - BrightSign has no built-in, software-only remote-control API the way Tizen does, so that signal doesn't carry over automatically. Equivalent input is achievable (a USB HID remote/RF receiver typically just works as ordinary keydown events; a GPIO-wired IR receiver enables custom hardware integration), but it requires hardware the deployment may or may not actually have - it's not a given. Without confirmed interactive hardware, default media playback to loop continuously (typical "set and forget" signage) rather than stopping after one playthrough.
...
```json
{
  ...
  "description": "Tizen apps are commonly built assuming a remote control as the interaction model - remote-driven menus, pause/resume, navigation. BrightSign has no built-in, software-only remote-control API the way Tizen does (see remote_key_handling category) - there's no free equivalent to just call. That means a Tizen app being remote-interactive is NOT evidence that the resulting BrightSign deployment is interactive; equivalent input is achievable via added hardware (a USB HID remote/RF receiver, which typically just works as ordinary keydown events, or a GPIO-wired IR receiver for custom integration), but only if that hardware is actually present. BrightSign digital signage defaults to 'set and forget': content plays continuously with no viewer present, unless the specific deployment adds real interactive hardware of its own - a touchscreen, GPIO buttons/sensors, USB HID remote, or similar.",
  "transformation_steps": [
    {
  ...
  "api_mappings": [
    {
      ...
      "notes": "Guard the Tizen call before removing it entirely. There is no built-in BrightSign software equivalent for color/channel buttons - unlike Tizen, there's no zero-hardware API to just call. Standard arrow/enter-style navigation maps cleanly to keyboard events regardless. Color/channel-style buttons specifically require added hardware: a USB HID remote/RF receiver (many present as a plain keyboard, so it just works) or a GPIO-wired IR receiver for custom integration - ask whether this deployment has or can add that hardware before assuming the functionality is simply unavailable.",
      "requires_user_decision": true,
      "ai_question": "AI_QUESTION: This app registers Tizen remote color/channel buttons, which have no built-in BrightSign equivalent. Is this functionality required? If so: does this deployment have (or can it add) a USB HID remote/RF receiver or a GPIO-wired IR receiver to provide real button input, or should it be redesigned as an on-screen control instead?"
    },
    {
      "category": "remote_key_handling",
    ...
    },
    {
      ...
      "notes": "Tizen apps are commonly built assuming a remote control as the interaction model. BrightSign has no built-in, software-only remote-control API the way Tizen does, so a Tizen app being remote-interactive is NOT automatic evidence the BrightSign deployment is interactive - that signal doesn't carry over by default. BrightSign signage defaults to 'set and forget': content plays continuously with no viewer present. The only thing that should override the loop default is confirmed interactive hardware on the BrightSign side itself - a touchscreen, GPIO buttons/sensors, a USB HID remote/RF receiver, or similar - which is a deployment fact, not something inferable from the Tizen source (BrightSign is modular here: a USB HID remote receiver typically just works as ordinary keydown events, and GPIO gives raw pin access for a custom IR receiver, but neither is present unless someone actually wired it up). Always ask whether such hardware is present if it hasn't been specified; do not infer interactivity (or the lack of it) from how the original Tizen app used its remote."
    },
    {
      "category": "media_playback",
...
```json
{
  "common_pitfalls": [
    ...
    },
    {
      ...
      "impact": "Content plays once and then sits on a dead frame instead of looping - the Tizen app's remote-driven interactivity doesn't carry over automatically (BrightSign has no built-in software equivalent, even though USB HID or GPIO hardware could provide one if actually present), so treating it as evidence the deployment is interactive silently produces exactly this bug",
...
```json
{
  "tizen_runtime_not_portable": {
    ...
    "limitations": [
      ...
      "IR remote color/channel button codes have no built-in BrightSign software equivalent - signage players don't ship with a remote receiver the way Tizen TVs do; equivalent input requires added USB HID or GPIO hardware",
